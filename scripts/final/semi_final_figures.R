...
# ggsave(plot = fig_1.2,
#        filename = here("figures", "mc_results", "final", "test.png"),
#        height = 4.5,
#        width = 12,
#        bg = "white")
...
(p2.2 <- ggplot(predictions_dominance[period == "night_time" & avg_dominance_sleep > 0.59 & avg_dominance_sleep < 0.61 & dominance == 0.5
    ...
    scale_color_manual(name = "",
                        labels = c(expression("Continue activity "(widehat(p)[A%->%A])),
                          ...
                          expression("Rest probability "(widehat(p)[R]))),
                         values = c("#F8766D", "#00BFC4", "black")) +
    ggtitle("B)") +
    xlab("Number individuals resting (excluding focal)") +
    ylab("Probability") +
...
ggsave(plot = fig_2,
       filename = here("figures", "mc_results", "final", "fig_2.png"),
       height = 3.4,
       width = 10,
       bg = "white")
...
(p3.1 <- predictions_dominance[period == "night_time" & avg_dominance_sleep > 0.49 & avg_dominance_sleep <= 0.51 & number_sleeping_excluding_focus == 6
  ...
  pivot_longer(cols = c("p_rest", "p_aa", "p_ass"),
   ...
   ylab("Probability") +
   ggtitle("A)") +
  theme(legend.position = c(0.0, 0.25)) +
  guides(color = guide_legend(override.aes = list(linewidth = 2))) +
   #background_grid() +
   xlab("Dominance"))

(p3.2 <- ggraph(net_null_model |>   
                  activate(edges)  |>
...
(p3.3 <- ggplot(predictions_dominance[number_sleeping_excluding_focus == 6]) +
...
(fig_3.1 <- ggarrange(p3.1, p3.2, p3.3,
                    ncol = 3, nrow = 1))

(p_3.4 <- ggplot(lk_network[id != id_pair][syncrony %in% c("syncro")
] |> merge(dominance_id)) +
...
(fig_3 <- ggarrange(fig_3.1, p_3.4,
                    ncol = 1, nrow = 2))

ggsave(plot = fig_3,
       filename = here("figures", "mc_results", "final", "fig_3.png"),
       height = 6,
       width = 13,
       bg = "white")
