# Figure 1 ----
p1.1 <- copy(combined_night_m)[, ":="(time = (as.numeric(seconds(time)) + 12 * 60 * 60) %% (24 * 60 * 60)
...
fig_1.1 <- ggarrange(p1.1, p1.2,
                      ncol = 1, nrow = 2)

(p1.3 <- ggplot(null_n_simultaneously_sleeping[, c("n", "prob_sleep", "freq_number_sleeping")] |>
                  pivot_longer(cols = c("prob_sleep", "freq_number_sleeping"),
                               names_to = "distribution",
                               values_to = "Probability")) +
    geom_col(aes(n / 12, Probability), fill = "black", alpha = 0.1,
             color = "black") + #, size =1
    scale_x_continuous(name = "Proportion sleeping") +
    scale_y_continuous(name = "Probability"#,
                       #limits = c(0, 0.25)
    ) +
    ggtitle("B)") +
    facet_grid(rows = vars(distribution),
               labeller = as_labeller(c("prob_sleep" = "Null model",
                                     "freq_number_sleeping" = "Observed"))) +
    background_grid(major = "y"))

(p1.4 <- predictions_dominance[avg_dominance_sleep > 0.59 & avg_dominance_sleep < 0.61 & dominance == 0.5
][, ":="(p_ass = 1 - p_sa,
         p_aa = 1 - p_as)
    ...
    pivot_longer(cols = c("p_rest", "p_aa", "p_ass"),
                 ...
                 values_to = "probability") |>
    ggplot() +
    geom_ribbon(data = predictions_dominance[avg_dominance_sleep > 0.59 & avg_dominance_sleep < 0.61 & dominance == 0.5],
    ...
    geom_line(data = predictions_dominance[ avg_dominance_sleep > 0.59 & avg_dominance_sleep < 0.61 & dominance == 0.5],
...
ggsave(plot = fig2,
       filename = here("figures", "submission", "fig_1.png"),
       height = 9,
       width = 9,
       bg = "white")
...
(p2.1 <- predictions_dominance[period == "night_time" & avg_dominance_sleep > 0.49 & avg_dominance_sleep <= 0.51 & number_sleeping_excluding_focus == 6
][, ":="(p_ass = 1 - p_sa,
         p_aa = 1 - p_as)
...
(p2.2 <- predictions_dominance[period == "night_time" & avg_dominance_sleep > 0.59 & avg_dominance_sleep < 0.61 & dominance %in% c(0, 0.5, 1)
][, ":="(pss = 1 - p_sa,
         paa = 1 - p_as)
...
(fig_2.4 <- time_correlation_s_all[id_focus != id_other & shift < 50 & shift > -50] |>
    ggplot() +
...
fig_2.2 <- ggarrange(p2.3, p2.4,
                     ncol = 1, nrow = 2)

fig_2.3 <- ggarrange(fig_2.2, fig_2.2,
                     ncol = 1, nrow = 2,
                     heights = c(1, 2))
...
(p2.6 <- ggplot(cross_correlation_stats[peak_time != 0][, .(proportion_influence = unique(proportion_influence),
                                                            size = .N),
    ...
    xlab("Dominance") +
    ggtitle("C)") +
    annotation_custom(ggplotGrob(p2.6.1), 
                      xmin = -0.05, xmax = 0.5, 
                      ymin = 0.55, ymax = 1.1))

fig_2.4 <- ggarrange(p2.5, p2.6,
                     ncol = 1, nrow = 2)

(fig_2 <- ggarrange(fig_2.3, fig_2.4,
                      ncol = 2, nrow = 1))

ggsave(plot = fig2,
       filename = here("figures", "submission", "fig_2.png"),
       height = 9,
       width = 9,
       bg = "white")
...
(p3.1 <- ggplot(unique(mechanism_united[, c("dom_diff", "gran_median", "weight_dist")])) +
...
(p3.2 <- ggplot(unique(mechanism_united)) +
...
(p3.3 <- ggplot(centrality_combined_predictions |>
...
(fig_3 <- ggarrange(p3.1, p3.2, p3.3,
                    ncol = 3, nrow = 1))

ggsave(plot = fig_3,
       filename = here("figures", "submission", "fig_3.png"),
       height = 9,
       width = 9,
       bg = "white")
