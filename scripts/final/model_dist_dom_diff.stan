data {
  ...
  vector[N] median_distance; // response
  vector[N] dom_diff; // predictor
...
model {
  ...
  // likelihood
  for(n in 1:N) {
    target += normal_lpdf(median_distance[n] | alpha + beta * dom_diff[n] + beta_sex[sex_pair[n]] + // fixed effects
