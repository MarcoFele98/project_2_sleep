...
(p3 <- ggplot(markov_data_m[transition %in% c("p3")]) +
  geom_point(aes(dominance, probability), color = "#00BFC4",
             size = 5) +
  geom_smooth(aes(dominance, probability, weight = tot),
              method="glm",  linewidth = 3, color = "#00BFC4", 
              method.args=list(family="binomial")) +
  ylab("Probability of rest interruption") +
  xlab("Dominance") +
  background_grid())

ggsave(plot = p3,
       filename = here("figures", "mean.VeBDAs threshold 0.03", "presentation", "interruption~dominance.png"),
       height = 3.5,
       width = 4,
       bg = "white")
...
(p14 <- ggplot(data_relevance[is_night == T & syncro == 2][, ":="(sex = ifelse(sex_type_focus == "m", "m", "f"))
...
ggsave(plot = p14,
       filename = here("figures", "mean.VeBDAs threshold 0.03", "presentation", "influence~dominance.png"),
       height = 3.5,
       width = 4,
       bg = "white")

(p15 <- ggplot(time_correlation_l[id_focus == "Shebeleza" & syncro == 0 & id_focus != id_other & shift <= 50 & shift >= -50 & is_night == T]) + # & shift <= 100 & shift >= -100
...
ggsave(plot = p15,
       filename = here("figures", "mean.VeBDAs threshold 0.03", "presentation", "influence_time_series~dominance.png"),
       height = 3.5,
       width = 5,
       bg = "white")

(p16 <- ggplot(time_correlation_l[id_focus == "Gabrielle" & syncro == 0 & id_focus != id_other & shift <= 50 & shift >= -50 & is_night == T]) + # & shift <= 100 & shift >= -100
    geom_vline(aes(xintercept = 0), linewidth = 3, lty = "dashed") +
    geom_point(aes(shift_peak_prob, peak_prob, color = baboon_other), size = 5) +
    geom_line(aes(shift, prob, 
                  color = baboon_other, group = interaction(is_night, id_other)), linewidth = 1) +
...
ggsave(plot = p16,
       filename = here("figures", "mean.VeBDAs threshold 0.03", "presentation", "influence_time_series~dominance_gabrielle.png"),
       height = 3.5,
       width = 5,
       bg = "white")
