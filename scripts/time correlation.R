    ...
    for(shift in 1:250) {
      print(shift)
        time_correlation <- rbind(time_correlation, 
                                  (head(combined_night_m[, c(id1, "is_night"), with = F], -shift)[, ":="(syncro = get(id1) + tail(combined_night_m[[id2]], -shift)),
                                  ][, -id1, with = F] |>
                                    table() |>
...
#      file = here("data/time-correlation_t0.03.RData"),
#      header = T)

load(here("data", "time-correlation_t0.03.RData"))
...
# Modify dataset ----
time_correlation_m <- copy(time_correlation_corrected)[, ":="(id_before = ifelse(id_before == "Sheb", "Shebeleza", id_before),
                                                    id_after = ifelse(id_after == "Sheb", "Shebeleza", id_after))
                                             ][, ":="(tot = sum(N)),
                                               by = list(id_before, id_after, is_night, shift)
                                               ][, ":="(prob = N/tot)] |> 
  merge(dominance_id[, c("id", "dominance", "sex_type")] |>
          rename(dominance_before = dominance,
                 sex_type_before = sex_type),
        by.x = "id_before",
        by.y = "id") |>
  merge(dominance_id[, c("id", "dominance", "sex_type")] |>
          rename(dominance_after = dominance,
                 sex_type_after = sex_type),
        by.x = "id_after",
        by.y = "id") |>
  mutate(id_after = factor(id_after, levels = idsRanked),
         id_before = factor(id_before, levels = idsRanked))

time_correlation_l <- rbind(copy(time_correlation_m |> rename(id_focus = id_after,
                                                              id_other = id_before,
                                                              sex_type_focus = sex_type_after,
                                                              sex_type_other = sex_type_before,
                                                              dominance_focus = dominance_after,
                                                              dominance_other = dominance_before))[, ":="(shift = -shift)],
                            time_correlation_m |> rename(id_focus = id_before,
                                                         id_other = id_after,
                                                         sex_type_focus = sex_type_before,
                                                         sex_type_other = sex_type_after,
  ...
  mutate(peak_prob = ifelse(syncro != 1, max(prob), min(prob)),
...
data_relevance <- unique(time_correlation_l[, c("id_focus", "id_other", "is_night", 
                                                ...
                                                "sex_type_focus", "sex_type_other", "peak_prob", 
...
ggplot(data_relevance[is_night == T & syncro == 2][, ":="(sex = ifelse(sex_type_focus == "m", "m", "f"))
][, ":="(sex = ifelse(is.na(sex), "f", sex))]) +
  geom_hline(aes(yintercept = 0), color = "grey", lty = "dashed", linewidth = 1) +
  geom_point(aes(dominance_focus, relevance, color = sex),
             size = 5) +
